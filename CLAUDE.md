...
- The Quest page is the one bold surface: raised buttons and stones with a solid bottom edge, all derived from the student's accent (`useQuestColors`). Amber is reserved for being behind, never red. The streak flame is the one exception to the accent rule: it uses real fire colors (a blue flame reads as a drop), and grows from a single outlined sticker flame into a three-tongue campfire at a 7-day streak (`FLAME_GROWS_AT` in `questUi.jsx`). Everywhere else stays calm; the nav streak chip is the only raised element outside `/quest`.
