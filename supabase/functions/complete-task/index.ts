...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('quest_tasks').update(taskUpdate).eq('id', taskId)
    ...
    await supabase.from('quests').update({ confidence_score: newScore }).eq('id', roadmapId)
    ...
    if (allDone && phase.status !== 'completed') {
      await supabase.from('quest_phases').update({ status: 'completed' }).eq('id', task.phase_id)
