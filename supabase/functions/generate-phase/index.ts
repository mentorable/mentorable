...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existingPhase } = await supabase
      .from('quest_phases')
      .select('*, tasks:quest_tasks(*)')
      .eq('quest_id', roadmapId)
...
${profile.quest_hours_per_week ? `- Weekly time available: ${profile.quest_hours_per_week} hours/week — size tasks accordingly` : ''}
${profile.quest_task_style && profile.quest_task_style !== 'mix' ? `- Preferred learning style: ${profile.quest_task_style === 'reading' ? 'reading & research' : profile.quest_task_style === 'hands_on' ? 'hands-on projects & building' : 'video-based learning'} — lean toward this format` : ''}
${profile.quest_difficulty ? `- Difficulty preference: ${profile.quest_difficulty} — ${profile.quest_difficulty === 'gradual' ? 'start easy and build up slowly' : profile.quest_difficulty === 'challenging' ? 'push them with harder tasks from the start' : 'balance accessible and challenging tasks'}` : ''}
