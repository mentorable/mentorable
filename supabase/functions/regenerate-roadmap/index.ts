...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { phaseId, phaseNumber, questId: incomingQuestId } = body
    ...
    const roadmapId = incomingQuestId || activeRoadmap?.id

    // ── PHASE-LEVEL REGENERATE ─────────────────────────────────────────────────────
    if (phaseId && phaseNumber && roadmapId) {
      // Delete existing tasks for the phase
      await supabase.from('quest_tasks').delete().eq('phase_id', phaseId)
      // Delete the phase row itself
      await supabase.from('quest_phases').delete().eq('id', phaseId)
...
- Pre-roadmap certainty: ${profile?.career_certainty || 'N/A'}
- Pre-roadmap career: ${profile?.career_direction || 'N/A'}
...
`,
    ...
    try {
      ...
      if (profile?.career_certainty === 'certain' && profile?.career_direction) {
        synthesis = { startingMode: 'career', careerDirection: profile.career_direction }
      }
    }

    // Deactivate current roadmap
    if (activeRoadmap) {
      await supabase.from('quests').update({ is_active: false }).eq('id', activeRoadmap.id)
    }
    ...
    if (synthesis.startingMode === 'career' && synthesis.careerDirection) {
      ...
      await supabase.from('profiles').update({ career_matches: updated, career_direction: synthesis.careerDirection }).eq('id', user.id)
    }
