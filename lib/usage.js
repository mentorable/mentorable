...
export async function fetchUsage(supabase) {
  const { data } = await supabase
    .from('usage_tracking')
    .select('chat_messages_used, research_queries_used, quest_generations_used')
    .maybeSingle()
  return data ?? { chat_messages_used: 0, research_queries_used: 0, quest_generations_used: 0 }
}
